...
new g_itemid, g_elemental[33], g_frozen[33], frost_spr, bullets[33], g_zoom[33], g_buy_limit
new tracer_spr, fire_spr, smoke_spr, g_burning[33], burn_time[33]

// Cvars
new cvar_frosttime, cvar_dmgmultiplier, cvar_burndmg, cvar_burntime, cvar_burn_nemesis_time, g_maxplayers,
...
{
	if(is_user_valid_alive(id) && zp_get_user_zombie(id) && !zp_get_user_nemesis(id) && !g_frozen[id])
	{
...
public set_user_burn(victim)
{
	if(is_user_valid_alive(victim) && zp_get_user_zombie(victim))
	{
		if(!g_burning[victim] && get_user_health(victim) > get_pcvar_num(cvar_minhealth))
		{
			...
			if(zp_get_user_nemesis(victim)) burn_time[victim] = get_pcvar_num(cvar_burn_nemesis_time)
