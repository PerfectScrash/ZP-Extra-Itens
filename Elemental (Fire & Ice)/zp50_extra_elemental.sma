...
{	
	...
	register_cvar("zp_elemental", "1.3", FCVAR_SERVER|FCVAR_UNLOGGED);
...
{
	if (!is_user_valid_alive(id) || zp_core_is_zombie(id)) return PLUGIN_HANDLED
...
{
	if(is_user_valid_alive(attacker) && !zp_core_is_zombie(attacker) && get_user_weapon(attacker) == CSW_ELITE && g_elemental[attacker] && is_user_valid_alive(victim) && zp_core_is_zombie(victim))
	{
		...
		switch(random_num(1,100))
		{
			case 1..30: 
			{
				...
				set_task(get_pcvar_float(cvar_fr_duration),"removefreeze",victim)
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(!is_user_valid_alive(id) || zp_core_is_zombie(id) || szWeapID != CSW_ELITE && g_zoom[id]) 
	{
		g_zoom[id] = false
...
public make_tracer(id)
{
	if (get_pcvar_num(cvar_tracer) && is_user_valid_alive(id))
	{
		...
		if(is_user_valid_alive(iVictim)) {
			if ((bullets[id] > clip) && (wpnid == CSW_ELITE) && g_elemental[id] && !zp_core_is_zombie(iVictim))
			{
...
public zp_fw_items_select_pre(id, itemid)
{
	...
	if(zp_core_is_zombie(id) || g_buy_limit >= get_pcvar_num(cvar_limit)) return ZP_ITEM_DONT_SHOW;
...
/*------------------------------------=[Remove fire and freeze]=-------------------------------------*/
public removefreeze(plr) {
	if(is_user_connected(plr)) 
		zp_grenade_frost_set(plr, false)
}
public removefire(plr) {
	if(is_user_connected(plr))
		zp_grenade_fire_set(plr, false)
}
